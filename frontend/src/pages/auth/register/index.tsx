...
import { emailPattern, registerUser, RegisterUserData } from "@/utils/auth/auth.utils";
  ...
  const onSubmit = async (values: RegisterUserData) => {
    ...
    if (values){
      try {
        ...
        enqueueSnackbar((
          <div>
            <strong className="font-bold">Usuario registrado con éxito</strong> <br />
            <span>Se ha enviado un correo de activación al tu bandeja de entrada.</span>
          </div>
        ), { variant: "success" });
        ...
        if (errors?.username) {
          const alreadyExists = errors.username.find(x => x.includes("already exists"));
          if (alreadyExists) {
            enqueueSnackbar((
              <div>
                <strong className="font-bold">Error al registrar al usuario</strong> <br />
        ...
        if (errors?.password1) {
          const commonPassword = errors.password1.find(x => x.includes("too common"));
          if (commonPassword) {
            enqueueSnackbar((
              <div>
                <strong className="font-bold">Contraseña muy común</strong> <br />
                <span>La contraseña utilizada es muy común y es propensa a ser fácilmente descifrada, por favor intenta con una diferente.</span>
              </div>
            ), { variant: "error" });
            return;
          }
        ...
        enqueueSnackbar((
          <div>
            ...
            <span>Los datos ingresados contienen errores, por favor corrígelos.</span>
          </div>
        ), { variant: "error" });
      }
      finally {
    ...
    <div className="h-full grid place-items-center">
      <div className="border border-1 rounded-xl p-6 max-w-2xl">
        ...
        >
          <div className="p-6">
            <p className="mb-4">
              <span className="text-green-500 text-2xl">{alert?.title}</span>
            </p>
            <p className="text-neutral-800 mb-8">{alert?.content}</p>
