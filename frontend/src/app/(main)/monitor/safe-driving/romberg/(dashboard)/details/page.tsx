...
import {
  useRombergSeverityCount,
  useRombergStatusQuery,
...
import RombergDeviceCard from "@/app/(main)/monitor/(components)/RombergDeviceCard";
...
const RombergDetailsPage = () => {
  ...
  const devicesQuery = useRombergStatusQuery({
    refetchOnWindowFocus: false,
  });
  const deviceData = devicesQuery.data;

  const countQuery = useRombergSeverityCount({
    refetchOnWindowFocus: false,
  });
    ...
    <section>
                ...
                {countQuery.data.map((severity_count) => (
                  ...
                  >
                    ...
                    <Link
                      href={
                        filter == null
                          ? "/monitor/safe-driving/romberg/details?" +
                            createQueryString(
                              "filter",
      ...
      <div className="flex flex-row gap-4 flex-wrap">
        {deviceData?.map((device, i) =>
          device.device_name.toLowerCase().includes(value.toLowerCase()) &&
          (filter == null || device.severity == Number(filter)) ? (
            <RombergDeviceCard key={i} device={device} />
          ) : null
        )}
      </div>
    </section>
...
export default RombergDetailsPage;
